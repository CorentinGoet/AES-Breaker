...
class AESUtils:
    ...
    """
    def __init__(self):
        self.SBox = [99, 124, 119, 123, 242, 107, 111, 197, 48, 1, 103, 43, 254, 215, 171, 118, 202, 130, 201, 125, 250, 89, 71,
    ...
    def addRoundKey(self, plaintext, key):
        """
        ...
        """
        key_tab = np.array([key]*16)
        return np.bitwise_xor(key_tab, plaintext)
    ...
    def subByte(self, initialBytes):
        """
        Opération de substitution d'octets.

        :param initialBytes: tableau de 16 octets d'entrée à substituer
        :return: Octets substitués
        """
        output = np.zeros(16, dtype=np.int32)
        for i in range(16):
            output[i] = self.SBox[initialBytes[i]]
        return output

    def invSubByte(self, finalBytes):
        """
        Substitution inverse par SBox.

        :param finalBytes: tableau de 16 octets de sortie de subByte
        :return: antécédents de finalBytes par SubByte
        """
        output = np.zeros(16, dtype=np.int32)
        for i in range(16):
            output[i] = self.SBox.index(finalBytes[i])
        return output
    ...
    def shiftRow(self, byteMat):
        ...
        :param byteMat: Matrice de 16 octets
        :return: Matrice résultat de la rotation
        """
        output = np.zeros(16, dtype=np.int32)
        for i in range(4):  # Indice de ligne
            for j in range(4):  # Indice de colonne
                ind_input = 4*i + ((i + j) % 4)
                ind_output = 4*i + j
                output[ind_output] = byteMat[ind_input]
        return output
    ...
    def invShiftRow(self, byteMat):
        """
        Opération inverse du ShiftRow

        :param byte: octet de sortie du shiftRow.
        :return: antécédent de l'entrée par shiftRow
        """
        output = np.zeros(16, dtype=np.int32)
        for i in range(4):  # Indice de ligne
            for j in range(4):  # Indice de colonne
